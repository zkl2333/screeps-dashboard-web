...
function extractPayloadError(payload: unknown): string | undefined {
  ...
  while (queue.length > 0) {
    ...
    const okValue = asNumber(record.ok);
    if (okValue === 0) {
...
function extractConsoleFeedback(payload: unknown): string | undefined {
  ...
  while (queue.length > 0) {
    ...
    if (typeof current === "string") {
      const feedback = sanitizeConsoleFeedback(current);
      if (feedback) {
        return feedback;
      }
      continue;
    ...
    const fromRecord = sanitizeConsoleFeedback(
      firstString([
        record.result,
        record.output,
        record.stdout,
...
export async function sendConsoleCommand(
  ...
  const shard = normalizeShard(shardInput);
  const baseBodies: Array<Record<string, unknown>> = [{ expression: trimmed }, { command: trimmed }];
